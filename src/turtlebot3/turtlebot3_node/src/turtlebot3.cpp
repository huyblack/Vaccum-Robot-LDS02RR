...
{
  XiaoBLEI2CWrapper::Device device;
  // Đọc tham số từ ROS param nếu có
  std::string i2c_device;
  int i2c_address;
  
  this->declare_parameter("i2c_device", "/dev/i2c-1");
  this->declare_parameter("i2c_address", 8);  // 0x08 trong hệ thập phân
  
  this->get_parameter("i2c_device", i2c_device);
  this->get_parameter("i2c_address", i2c_address);
  
  device.i2c_device = i2c_device;
  device.i2c_addr = static_cast<uint8_t>(i2c_address);
  
  // Tạo đối tượng XiaoBLEI2CWrapper
  dxl_sdk_wrapper_ = std::make_shared<XiaoBLEI2CWrapper>(device);
...
void TurtleBot3::add_wheels()
{
  ...
  this->get_parameter_or<float>("wheels.separation", wheels_.separation, 0.24);
  this->get_parameter_or<float>("wheels.radius", wheels_.radius, 0.035);
}
  ...
  cmd_vel_sub_ = std::make_unique<TwistSubscriber>(
    ...
    std::function<void(const geometry_msgs::msg::Twist::SharedPtr)>(
      [this](const geometry_msgs::msg::Twist::SharedPtr msg) -> void
      {
        ...
        union Data {
            float fdata[6];  // Đổi từ int32_t thành float
            uint8_t byte[4 * 6];
        } data;
        ...
        data.fdata[3] = 0.0f;
        data.fdata[4] = 0.0f;
        data.fdata[5] = msg->angular.z;  // Bỏ nhân với 100
