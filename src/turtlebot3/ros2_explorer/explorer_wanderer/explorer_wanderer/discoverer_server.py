...
# messages
# from std_msgs.msg import Float32  # Bỏ vì không còn sử dụng map_progress
    ...
    for y in range(height):
        for x in range(width):
            ...
            if map_array[idx] <= free_thresh and map_array[idx] > -1:
                # Kiểm tra xem có phải frontier không (có unknown cells lân cận)
                is_frontier = False
                for dx, dy in [(-1,0),(1,0),(0,-1),(0,1)]:
                    nx, ny = x+dx, y+dy
                    if 0 <= nx < width and 0 <= ny < height:
                        nidx = ny * width + nx
                        if map_array[nidx] == -1:  # Unknown cell
                            is_frontier = True
                            break
...
class DiscovererServer(Node):
    def __init__(self):
        ...
        # Thay ActionServer bằng Service
        self._service = self.create_service(Trigger, 'discover', self.execute_callback)
        ...
        self.frontier_threshold = 3  # Giảm xuống 3 để robot dừng khi ít frontier
        self.frontier_count = 1 
        self.create_subscription(
            OccupancyGrid, 'map', self.map_callback, 10
        )
        # Biến theo dõi frontier
        self.last_frontier_count = 1
        # Thêm biến theo dõi thời gian tối thiểu
    ...
    def execute_callback(self, request, response):
        ...
        # Đảm bảo robot luôn chạy ít nhất một lần
        first_iteration = True
        
        # Logic mới: Dừng khi frontier count < threshold VÀ đã chạy đủ thời gian tối thiểu
        while (first_iteration or 
               (self.frontier_count >= self.frontier_threshold or 
                time.time() - self.start_time < self.min_exploration_time)) and iteration_count < max_iterations:
            ...
            self.get_logger().info(f"Time elapsed: {current_time:.1f}s")
            self.get_logger().info(f"First iteration: {first_iteration}, Frontier >= threshold: {self.frontier_count >= self.frontier_threshold}, Time < min: {current_time < self.min_exploration_time}")
            
            # Cập nhật frontier count
            ...
            iteration_count += 1
            first_iteration = False  # Sau lần đầu tiên, tắt flag này
    ...
    def map_callback(self, msg):
        ...
        # Tính toán thống kê map (chỉ để log, không dùng để quyết định dừng)
        total_cells = len(map_array)
        free_cells = np.sum(map_array <= 0.25)
        occupied_cells = np.sum(map_array > 50)
        unknown_cells = np.sum(map_array == -1)
        explored_percentage = (free_cells + occupied_cells) / total_cells * 100
        
        # Log thay đổi frontier_count và thống kê map
        if old_frontier_count != self.frontier_count:
            ...
            self.get_logger().info(f'Map stats - Explored: {explored_percentage:.1f}%, Free: {free_cells}, Occupied: {occupied_cells}, Unknown: {unknown_cells}')
        
        # Log định kỳ mỗi 3 lần callback để theo dõi thường xuyên hơn
        if hasattr(self, '_callback_count'):
            self._callback_count += 1
        else:
            self._callback_count = 0
            
        if self._callback_count % 3 == 0:
            self.get_logger().info(f'Map update - Frontiers: {self.frontier_count}, Explored: {explored_percentage:.1f}%')
        
        # Log ngay lập tức nếu frontier count = 0
        if self.frontier_count == 0:
            self.get_logger().warn(f'WARNING: Frontier count is 0! Map explored: {explored_percentage:.1f}%')
...
class CartographerSubscriber(Node):
    ...
    def convolute(data, coordinates, size=3, threshold=40):
        ...
        """
        sum_val = 0.0  # Sử dụng float để tránh overflow
        for x in range(int(coordinates[0] - size / 2), int(coordinates[0] + size / 2)):
            for y in range(int(coordinates[1] - size / 2), int(coordinates[1] + size / 2)):
                ...
                if data[x, y] == -1:
                    sum_val += 100.0
                # if occupancy state is above 50 (occupied), we add 1000 to the sum so that the robot DOES NOT
                # access areas near walls. (Giảm từ 1000000 xuống 1000 để tránh overflow)
                elif data[x, y] > 50:
                    sum_val += 1000.0
                # if the occupancy state is below 50 and known, just add the value to sum.
                else:
                    sum_val += float(data[x, y])

        # average value for the square is computed
        average = sum_val / (size * size)
